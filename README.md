...
* `Node.js version >= 7.10.1`: для запуска тестов сервера нужна нода, поддерживающая `ES2016` и часть `ES2017`: `async/await`
...
`npm run build` - очистит папку `www`, прогонит тесты и соберт бандл и `index.html` в папке `www`

# Описание приложения

Прилоежние представляет из себя `Single Page Application` запускаемое в браузере.

Приложение позволяет просматривать список, добавлять и редактировать сообщения.
